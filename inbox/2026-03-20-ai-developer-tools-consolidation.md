...
## 🔥 今日重點 Top Highlights
- **OpenAI 收購 Python 工具公司 Astral**，正式進軍開發者工具領域，可能影響未來 AI 編程生態。[Link](https://astral.sh/blog/openai)
- **Anthropic 對 OpenCode 提起訴訟**，凸顯 AI 開源模型使用與授權的持續緊張局勢，開發者需注意合規風險。[Link](https://github.com/anomalyco/opencode/pull/18186)
- **Google 公佈 Android 側載新規**，未來安裝 Play Store 外應用需等待 24 小時，旨在提升安全性。[Link](https://arstechnica.com/gadgets/2026/03/google-details-new-24-hour-process-to-sideload-unverified-android-apps/)
- **攝影學習：手動控制色溫營造氛圍**，將白平衡設定自訂按鈕（如 Sony A7C），可快速調整畫面冷暖情緒。
- **LeetCode 重點：Trie 結構處理萬用字元搜尋**，掌握 211 題的 Trie + DFS 模式，對解決字串搜尋問題至關重要。

今日摘要狀態：
- 💻 Tech: OpenAI 收購 Astral、Anthropic 提起訴訟、Google Android 側載新規、AI 編碼代理與影片學習研究。
- 🧩 LeetCode Blind 100: 完成 #211 設計支援萬用字元搜尋的資料結構 (Trie + DFS)。
- 📷 Learning — Photography: 學習運用冷暖色溫（白平衡）來主導攝影畫面的情緒與氛圍。
- 📚 Learning — Tech: 比較藍綠部署與金絲雀部署的適用場景與實作注意事項。
- 🎬 Learning — YouTube: 學習如何保持影片標題與縮圖文字的一致性邏輯，以提升點擊率。
