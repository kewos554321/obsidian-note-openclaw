...
sections: ["tech_ai", "learning_leetcode", "learning_photography", "learning_tech", "learning_youtube"]
---
...
## 🔥 今日重點 Top Highlights
- **OpenAI 收購 Python 工具公司 Astral**：OpenAI 正式進軍開發者工具領域，收購了知名 Python 工具鏈公司（Ruff 的開發者），這可能預示著未來 AI 與開發流程更深度的整合。[Link](https://astral.sh/blog/openai)
- **Anthropic 對 OpenCode 提起訴訟**：圍繞開源 AI 模型使用與授權的爭議升級，此案結果可能對開發者社群使用類似模型的方式產生影響。[Link](https://github.com/anomalyco/opencode/pull/18186)
- **Google 公佈 Android 側載新規**：為提升安全性，安裝 Play Store 外未驗證應用的流程將變為需等待 24 小時，這可能影響喜歡嘗試新興或地區性應用的用戶。[Link](https://arstechnica.com/gadgets/2026/03/google-details-new-24-hour-process-to-sideload-unverified-android-apps/)
- **LeetCode 重點：萬用字元搜尋的 Trie 實作**：今日 Blind 100 題目（211. Design Add and Search Words Data Structure）是掌握 Trie 資料結構與回溯法的關鍵模式，附上 Python 詳解。
- **攝影學習：手動控制色溫營造氛圍**：學會手動設定白平衡（如 Sony A7C 自訂按鍵快速調整）來主導畫面情緒（暖調親密 vs. 冷調孤寂），避免場景中色溫混亂。

---
今日摘要狀態：
- 💻 Tech: OpenAI 收購 Astral、Anthropic 提起訴訟、Google Android 側載新規、AI 編碼代理研究、Meta V-JEPA 2.1 模型。
- 🧩 LeetCode Blind 100: 完成第 211 題（Trie + 萬用字元搜尋），包含解法與模式分析。
- 📷 Learning — Photography: 學習運用色溫（暖/冷）來營造攝影畫面的情緒與氛圍。
- 📚 Learning — Tech: 比較 Blue-Green 與 Canary 兩種部署策略的適用場景與注意事項。
- 🎬 Learning — YouTube: 學習如何保持影片標題與縮圖文字的一致性邏輯以提升點擊率。
